...
\title{Leaf: Machine-Checked Fairness Certificates\\via Trustworthy Probabilistic Typed Natural Deduction}

\author[1]{Fabio Aurelio D'Asaro}[
  orcid=0000-0002-6254-2372,
  email=fabio.dasaro@unimi.it]

\address[1]{Department of Philosophy ``Piero Martinetti,''
  University of Milan, Italy}

\begin{abstract}
We present \emph{Leaf}, a Lean~4 implementation of a core fragment of
Trustworthy Probabilistic Typed Natural Deduction (TPTND).
Leaf type-checks derivation trees whose leaves are empirical observations
...
All arithmetic uses exact rationals; confidence intervals
use the score-test approximation.
We extend the original one-sample trust rules with two-sample
variants \textsc{IT2}/\textsc{IUT2} that compare two observed
populations directly, and we describe the four-stage workflow that
takes group-level summary statistics through claim selection and
derivation construction to a verified certificate.
Two case studies on public data illustrate the approach:
(i)~the ProPublica COMPAS recidivism data (6\,172 defendants,
reproducing the headline racial-bias finding as a certified
\UTrust{} derivation), and
(ii)~HMDA mortgage-lending data for Delaware
(55\,000 originated-or-denied applications across 2022--2023, demonstrating
multi-year temporal composition via \textsc{Update},
intersectional analysis, and depth-4 derivation chains).
The full source, data pipelines, and all derivations are
publicly available at \url{https://github.com/dasaro/leaf-tptnd}.
\end{abstract}
...
In this paper we describe \textbf{Leaf}, a Lean~4 implementation
...
\item \textbf{Two case studies} on public data
  ...
  mortgage lending, producing derivation trees of depth up to~4.
\end{enumerate}
...
rules reconcile the two via score-test confidence intervals
$\CI(n,f,p)$.
...
\noindent
The one-sample confidence interval is
\(
  \CI(n,f,p) = f \pm z_{.975}\sqrt{p(1{-}p)/n},
\)
clamped to $[0,1]$, where $z_{.975}=1.96$.
The two-sample interval is
\(
  \CItwo(n,m,f,g) = (f{-}g) \pm z_{.975}\sqrt{\hat p(1{-}\hat p)(1/n+1/m)},
\)
with $\hat p = (nf{+}mg)/(n{+}m)$.
...
in~\cite{dasaro2025tptnd} and are implemented in Leaf but not
...
% ====================================================================
\section{Leaf: Implementation in Lean 4}\label{sec:impl}
% ====================================================================

\subsection{Kernel}\label{sec:arch}

Leaf is approximately 1\,700 lines of Lean~4: a read-only kernel
...
failing premise.
Three type-system features keep the kernel sound by construction.
A probability is wrapped in a $\mathtt{Prob}$ structure carrying
proof witnesses of $0\le q\le 1$ at construction time, so an
ill-formed probability is unrepresentable.  The checking monad
forces every rule to return either a checked result or an
explicit error: silent failures are not expressible.  Rule names
are a closed inductive datatype, so the dispatcher is
exhaustively type-checked and a misspelled rule is a
compile-time, not a runtime, error.
$\texttt{checkDerivation}$ is declared \texttt{partial} because
the recursion over the premise list is opaque to Lean's
structural-recursion checker; a maliciously crafted cyclic
``tree'' would therefore not terminate, and a well-founded
variant on tree depth is left for future work.  In every
case-study derivation the recursion returns immediately.
...
$49/25 = 1.96$.
%
The square-root rounding is the one place where Leaf is not bit-exact:
...
decidable by Leaf; we treat it as a known soundness boundary
...
\]
would derive a probability of $3/2$.  In Leaf the rule layer
...
An auditor produces a Leaf certificate in four stages.

\smallskip\noindent
\textbf{1. Group-level summary statistics.}
The auditor extracts sample sizes and event counts for the
demographic strata of interest.  No Leaf-specific tooling is
...
The fairness question fixes which Leaf rule closes the
...
\item \textbf{Predictive parity / calibration}~\cite{chouldechova2017fair}
  ...
  Chouldechova's incompatibility result implies that the
  equalized-odds violation we certify on COMPAS
  (Section~\ref{sec:compas}) cannot coexist with calibration on
  the same data, since racial base rates differ.
\end{itemize}
...
The Leaf-verified derivation is:
%
\[
\scriptstyle
\inferrule{
...
We build seven derivation trees, organised in five cases that
...
\]
Since $0 \notin [0.1357, 0.1528]$, it yields \UTrust{}.

\smallskip\noindent
\textbf{Tree~C: full chain (depth~4).}\;
\textsc{EUT} appends a single step to Tree~B's
$\UTrust_\CItwo$ certificate, extracting the observed Black
frequency into a term claim that carries the complement interval
$\lnot[0.1357, 0.1528]$ in its typing context:
\[
\scriptstyle
\inferrule{
  \Gamma \vdash \UTrust_\CItwo(t_{12536}:\Denied_{4444/12536};\;
    8822/41969,\, [0.1357, 0.1528])
}{
  \Gamma, x_u:\Denied_{\lnot[0.1357, 0.1528]}
    \vdash_{\sigma_B} t_{12536}:\Denied_{4444/12536}
}\;\smark{EUT}
\]
where $\Gamma$ is the joint context of the two pooled cohorts
inherited from Tree~B.  The depth-4 chain
(\textsc{Obs}$\to$\textsc{Update}$\to$\textsc{IUT2}$\to$\textsc{EUT})
is verified end-to-end by a single \texttt{checkDerivation} call.

\smallskip\noindent
\textbf{Trees~D.1/D.2: intersectional certificates (depth~2).}\;
...
\smallskip\noindent
\textbf{Trees~E.1/E.2: year-over-year monitoring (depth~2).}\;
...
\midrule
HMDA   & Benchmark & \smark{IT} & 2 & \Trust \\
       & Temporal pool & \smark{Upd.+IUT2} & 3 & \UTrust \\
       & Full chain & \smark{Upd.+IUT2+EUT} & 4 & extracted \\
...
properties \emph{a priori} (of the model); Leaf instead produces
...
produce formally verified certificates: Leaf's architectural
...
We have presented Leaf, a Lean~4 implementation of a core TPTND
